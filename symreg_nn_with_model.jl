...
# Read data
A = readdlm(joinpath("data", "estimates", "order_$(nstate)_only_nn.csv"), ',')
p = A[:, 1:1]'
dx = A[:, 2:nstate+1]'
...
state = states(result(res_dyn))
f, f! = build_function(expr, state; expression=Val(false))
...
prob_extended = ODEProblem(wk4p_extended, x[:, 1], (0, tv[end]), (A, B))
